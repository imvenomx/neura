...
const getHtmlEntries = () => {
  ...
  // Get HTML files from /it/ subdirectory
  const itDir = path.resolve(__dirname, 'it');
  if (fs.existsSync(itDir)) {
    const itFiles = fs.readdirSync(itDir);
    const itHtmlFiles = itFiles.filter((file) => file.endsWith('.html'));
    itHtmlFiles.forEach((file) => {
      const name = `it/${path.basename(file, '.html')}`;
      entries[name] = path.resolve(itDir, file);
    });
  }
